...
def get_pipeline_stats(log_file):
    ...
    tot_mapped = re.sub(r".*Q >= \d+ \(", "", tot_mapped)
...
def write_pipeline_stats(stats, out_file):
    ...
    if file_extension == ".json":
        ...
        with open(out_file, "w") as stats_file:
            stats_file.write(f"Sample:             {prefix}\n")
            stats_file.write("Total read pairs:   {reads}\n".format(reads=f"{fastq_pairs:,}"))
            stats_file.write(
                "Mapped reads:       {tot_mapped} ({pct_mapped}%  of all reads)\n".format(
                    tot_mapped=f"{tot_mapped:,}", pct_mapped=pct_mapped
                )
            )
            stats_file.write(
                "Unmapped reads:     {tot_unmapped} ({pct_unmapped}%  of all reads)\n".format(
                    tot_unmapped=f"{tot_unmapped:,}", pct_unmapped=pct_unmapped
                )
            )
            stats_file.write(
                "Recovered contacts: {tot_pairs} ({pct_pairs}%  of all read pairs)\n".format(
                    tot_pairs=f"{tot_pairs:,}", pct_pairs=pct_pairs
                )
            )
            stats_file.write(
                "Removed contacts:   {removed_pairs} ({pct_removed}% of all contacts)\n".format(
                    removed_pairs=f"{removed_pairs:,}", pct_removed=pct_removed
                )
            )
            stats_file.write(
                "  Filtered out:     {filtered_pairs} ({pct_filtered}% of all contacts)\n".format(
                    filtered_pairs=f"{filtered_pairs:,}", pct_filtered=pct_filtered
                )
            )
            stats_file.write(
                "    Loops:          {loops_pairs} ({pct_loops_pairs}% of all contacts)\n".format(
                    loops_pairs=f"{loops_pairs:,}", pct_loops_pairs=pct_loops_pairs
                )
            )
            stats_file.write(
                "    Uncuts:         {uncuts_pairs} ({pct_uncuts_pairs}% of all contacts)\n".format(
                    uncuts_pairs=f"{uncuts_pairs:,}", pct_uncuts_pairs=pct_uncuts_pairs
                )
            )
            stats_file.write(
                "    Weirds:         {abnormal_pairs} ({pct_abnormal_pairs}% of all contacts)\n".format(
                    abnormal_pairs=f"{abnormal_pairs:,}",
                    pct_abnormal_pairs=pct_abnormal_pairs,
                )
            )
            stats_file.write(
                "  PCR duplicates:   {pcr_pairs} ({pct_pcr}% of all contacts)\n".format(
                    pcr_pairs=f"{pcr_pairs:,}", pct_pcr=pct_pcr
                )
            )
            stats_file.write(
                "Final contacts:     {final_pairs} ({pct_final}% of all contacts)\n".format(
                    final_pairs=f"{final_pairs:,}", pct_final=pct_final
                )
            )
